...
public class PostCategoryServiceImpl implements PostCategoryService {
    ...
    public void deletePostCategory(Long id) {
        var postCategory = postCategoryRepository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("Không tìm thấy thể loại bài viết để xóa"));
        
        if (postCategory.getPosts() != null && !postCategory.getPosts().isEmpty()) {
            throw new InvalidValueException("Không thể xóa thể loại bài viết này vì đang được sử dụng bởi " 
                + postCategory.getPosts().size() + " bài viết");
        }
